	...
	const thermalZones = await Promise.all(
		dirs
			...
			.map(async (dir) => {
				...
				try {
					const [typeName, tempStr] = await Promise.all([fs.read(typeFile), fs.read(tempFile)]);
					const temp = Number.parseInt(tempStr.trim().replace(/\D/g, ''), 10) / 1000;
					...
					console.warn(`Warning: Error reading files in ${dir.name}: ${err.message}`);
				}
			}),
	);
...
return baseclass.extend({
	...
	load: () =>
		Promise.all([
			fs.lines('/sys/kernel/debug/qca-nss-drv/stats/cpu_load_ubi').then((lines) => {
	...
	render: (data) => {
		...
		return E('div', {}, [E('div', {}, [loadSection]), E('div', {}, [tempSection])]);
	},
});
